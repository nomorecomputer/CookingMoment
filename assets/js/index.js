...
function registry_categoryMenu(
  ...
  if (!menuContainer || !menuItems || !ellipsis) return;

  window.addEventListener("resize", () =>
    debounce(Reset_Category_menu, 250)(menuContainerId, itemClassName)
  );
  ...
  ellipsis.addEventListener("click", function () {
    if (category_ellipsis_toggle == "show front") {
      let frontTwo = Array.from(
        document
          .getElementById(menuContainerId)
          .querySelectorAll("." + itemClassName)
      ).slice(0, 2);
      frontTwo.forEach((elem) => {
        elem.style.display = "none";
      });
      category_ellipsis_toggle = "show end";
    } else if (category_ellipsis_toggle == "show end") {
      let frontTwo = Array.from(
        document
          .getElementById(menuContainerId)
          .querySelectorAll("." + itemClassName)
      ...
      category_ellipsis_toggle = "show front";
    }
  });
}
// ************************************************************
...
function Reset_Category_menu(category_containerId, menuClassname) {
  if (window.innerWidth < 768) return;
  let menuItems = document
    .getElementById(category_containerId)
    .querySelectorAll("." + menuClassname);
...
function handle_banner_rotate(direction) {
  ...
  const imagePath = `url(/CookingMoment/assets/images/index/${imageName})`;
